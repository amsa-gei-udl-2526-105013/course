...
*Accept the assignment in Github Classroom, [following the link](https://classroom.github.com/a/3DHX45S9)*
...
How does our API work? First, [you have to setup your environment and start the server, see section 6](04-project.qmd#sec-setup).
...
![](../../figs/projects/04-project/example-server.png)
...
![](../../figs/projects/04-project/internal-example.png)
...
Finally, remember to create a cloud stack that allows to automatically recreate your architecture. It is important to include the minimun required to run your archictecture, **all the innecesary** or **not justified resources will be penalized**.

#### Description

Your goal is to **migrate the dockerized API** developed in **PRAC-4.1** and the **container orchestration** implemented in **PRAC-4.2** to the **AWS cloud**.  

![](../../figs/projects/04-project/act-4-3-blackbox.png)

The API’s database layer must be migrated to **Amazon RDS**, using **PostgreSQL (engine version 18.1-R1)**.  
For container deployment, you will use **Amazon Elastic Container Registry (ECR)** to store your images and **Amazon Elastic Container Service (ECS)** for execution.

**You are strictly forbidden to deploy using a raw EC2 instance running Docker.**  
The deployment **must** follow the network architecture shown in the diagram and **cannot be modified under any circumstances**.


#### Architecture Requirements & Tips

###### ECS Configuration
- **Launch Type:** `FARGATE` (mandatory)
- **Operating System / Architecture:**  
  - `Linux/X86_64` if the container image was built on an x86_64 machine  
  - `Linux/ARM64` if built on Apple Silicon (M1/M2/M3/M4) or Windows Snapdragon  
- **Logging:** Keep all default logging settings  
- **Cluster:** Deploy the task in **`amsa2526-ECSCluster`**  
- **Auto-Scaling:**  
  - Enable service auto-scaling  
  - Allow scaling up to **5 running tasks** during high-demand periods  
- **Networking:** Enable **Public IP assignment**  
- **Registry:** Push the final API image to **Amazon ECR**


###### **Database Migration (RDS)**

- Use **Amazon RDS – PostgreSQL (18.1-R1)**.  
- Configure your API to connect to the RDS instance following the prescribed network architecture.


###### **Mandatory Network Architecture**

The final deployment **must match exactly** the structure illustrated in the diagram below.  
**No modifications are allowed.**

![](../../figs/projects/04-project/act_4_full_schema.png)
...
Remember that, once you entered the Github Classroom on [section 2](04-project.qmd#delivery), you will have to clone your repository with:
